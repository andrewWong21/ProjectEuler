...
function isWinner1(game){
	...
	const hand1 = evaluate(cards.slice(0, 5));
	const hand2 = evaluate(cards.slice(5, 10));
	
	if (hand1.handRank == hand2.handRank){
	    const values = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A'];
	    return values.indexOf(hand1.highCard) > values.indexOf(hand2.highCard);
	}
...
function sortCards(hand){
    ...
    for (let card of hand){
        cards.push({rank : card[0], suit : card[1]})
    }
    const values = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A'];
    cards.sort((a, b) => values.indexOf(a.rank) - values.indexOf(b.rank));
    return cards;
}
...
function getCounts(cards){
    ...
    for (let card of cards){
        if (!counts[card.rank]) counts[card.rank] = 1;
        else counts[card.rank]++;
    }
    return counts;
}
...
function checkFlush(cards){
    for (let card of cards){
        if (card.suit != cards[0].suit) return false;
    }
    return true;
}
...
function checkStraight(cards){
    ...
    let firstFour = true;
    for (let i = 0; i < 4; i++){
        if (cards[i].rank != values[i]){
            firstFour = false;
            break;
        }
    }
    if (cards[4].rank == 'A' && firstFour){
        return true;
    }
    const idx = cards.map(card => values.indexOf(card.rank));
    for (let i = 0; i < 4; i++){
