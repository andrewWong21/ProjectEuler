...
public class _54_PokerHands{
	public static int Euler54(){
		...
		foreach (string match in matches){
			string[] cards = match.Split(' ');
			// first 5 cards are first hand
			List<string> hand1 = cards.Take(5).ToList();
			// last 5 cards are second hand
			List<string> hand2 = cards.Skip(5).Take(5).ToList();
			if (CompareHands(hand1, hand2) > 0) res++;
		}
		return res;
	}
	
	public static int CompareHands(List<string> h1, List<string> h2){
		List<int> score1 = EvaluateHand(h1);
		List<int> score2 = EvaluateHand(h2);
	...
	public static List<int> EvaluateHand(List<string> hand){
		List<int[]> cards = hand.Select(
			c => new int[2]{
				"23456789TJQKA".IndexOf(c[0]),
				"CDHS".IndexOf(c[1])
			}
		).ToList();
		cards.Sort((a, b) => a[0] - b[0]);
		
		List<int> ranks = cards.Select(c => c[0]).ToList();
		List<int> suits = cards.Select(c => c[1]).ToList();
		int[] rankCount = new int[13];
		foreach (int rank in ranks) rankCount[rank]++;
		
		bool isFlush = suits.Distinct().Count() == 1;
		bool isStraight = IsStraight(ranks);
		// Straight Flush
		if (isFlush && IsStraight(ranks)){
			return new List<int>{9, cards.Last()[0]};
		}
		// Four of a Kind
		if (rankCount.Contains(4)){
			int fourRank = Array.IndexOf<int>(rankCount, 4);
			return new List<int>{8, fourRank};
		}
		// Full House
		if (rankCount.Contains(3) && rankCount.Contains(2)){
			// get rank of triplet, then pair
			int threeRank = Array.IndexOf<int>(rankCount, 3);
			int twoRank = Array.IndexOf<int>(rankCount, 2);
			return new List<int>{7, threeRank, twoRank};
		}
		if (isFlush){
			return new List<int>{6, ranks.Last()};
		}
		if (isStraight){
			return new List<int>{5, ranks.Last()};
		}
		// Three of a Kind
		if (rankCount.Contains(3)){
			// get rank of triplet
			int threeRank = Array.IndexOf<int>(rankCount, 3);
			return new List<int>{4, threeRank};
		}
		// Two Pair
		if (rankCount.Count(x => x == 2) == 2){
			// get rank of higher pair
			var pairs = rankCount
				.Select((count, index) => new {rank = index, count})
				.Where(x => x.count == 2)
				.OrderByDescending(x => x.rank)
				.ToList();
			return new List<int>{3, pairs[0].rank, pairs[1].rank};
		}
		// Pair
		if (rankCount.Contains(2)){
			// get rank of pair
			int twoRank = Array.IndexOf<int>(rankCount, 2);
			return new List<int>{2, twoRank};
		}
		// High Card
		return new List<int>{1, ranks.Last()};
	}
	
	public static bool IsStraight(List<int> ranks){
		// check first four values (2, 3, 4, 5)
		...
		// 2, 3, 4, 5, A
		if (firstFour && ranks.Last() == 12) return true;
		// check if last four cards are 1 higher than previous card
		for (int i = 1; i <= 4; i++){
